...
S4LvariableListing <- list(
    "folder" = "boeings/stefan.boeing/214_BRD4_DN_RNAseq_analysis",
    "sra.id.vector" = "SRP095194",
    "gse.id.vector" = "GSE92456",
...
dfGSE$sample.id <- gsub(" ", "_", dfGSE$sample.id)
dfGSE$sample.id <- gsub("_rep", "_", dfGSE$sample.id)
...
dfDesign[["comp_1"]] <- ""
dfDesign[dfDesign$sample.group == "luc_JQ1","comp_1"] <- "1_JQ1"
dfDesign[dfDesign$sample.group == "luc_DMSO","comp_1"] <- "2_DMSO"

dfDesign[["comp_2"]] <- ""
dfDesign[dfDesign$sample.group == "dnBrd4_f3","comp_2"] <- "1_f3"
dfDesign[dfDesign$sample.group == "luc_DMSO","comp_2"] <- "2_DMSO"

dfDesign[["comp_3"]] <- ""
dfDesign[dfDesign$sample.group == "dnBrd4_f9","comp_3"] <- "1_f9"
dfDesign[dfDesign$sample.group == "luc_DMSO","comp_3"] <- "2_DMSO"
...
# Reorder samples in df.summary
new.order <- names(df.summary)

df.summary <- df.summary[,new.order]
...
database.table2$logFC_cut_off <- NULL
...
createNewProject(
    ...
    dbtable = "project_description_table",
    password = db.pwd,
    project_name = "BRD4 Inhibitors",
    project_lab = ";Portfolio;",
    project_description = "In this project the effect of BRD4 inhibitors on transcription is examined"
)
...
addProject2ProjectTable(
   ...
   experiment_question = "The expression of which genes is affect by the BRD4 inhibitor JQ1 as well as by dominant negative BRD4 constructs?",
   experiment_description = "<b>RNA Seq analysis</b>. Raji were treated with JQ1, DMSO or a dominant negative BRD4 construct was expressed. Cells were then subjected to RNA-Seq analysis.",
   experiment_owner = ";Eick lab;",
   experiment_lab = paste0(";",labname,";"),
   experiment_viewers = paste0(";", paste(user_ids, collapse = ";"),";"),
   experiment_project = ";Portfolio;",
